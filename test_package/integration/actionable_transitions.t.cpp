...
SCENARIO("action transition table", "[unit][variant]") {
  ...
  GIVEN("red") {
    ...
    auto increment_transition_count = [&transition_count](timer_expired const &e
                                                          [[maybe_unused]]) {
      ++transition_count;
    };
    transition_table target{
        dispatcher{},
        make_action_transition<red, green, timer_expired>(
            increment_transition_count),
        make_action_transition<green, yellow, timer_expired>(
            increment_transition_count),
        make_action_transition<yellow, red, timer_expired>(
            increment_transition_count)};
