...
TEST_CASE("ancestors with machine", "[unit][ancestry]") {
  SECTION("no states") {
    using target_type =
        ancestors<details_::dummy_machine<states_list<red, purple, blue>,
    ...
    REQUIRE_FALSE(is_full_ancestry<target_type>::value);
    REQUIRE_FALSE(concepts::full_ancestry<target_type>);
    REQUIRE(is_machine_ancestry<target_type>::value);
    REQUIRE(concepts::machine_ancestry<target_type>);
    ...
    SECTION("first generation") {
      ...
      REQUIRE_FALSE(is_full_ancestry<child_target_type>::value);
      REQUIRE_FALSE(concepts::full_ancestry<child_target_type>);
      REQUIRE(is_machine_ancestry<child_target_type>::value);
      REQUIRE(concepts::machine_ancestry<child_target_type>);
      ...
      SECTION("second generation") {
        ...
        REQUIRE(is_full_ancestry<grandchild_target_type>::value);
        REQUIRE(concepts::full_ancestry<grandchild_target_type>);
        REQUIRE(is_machine_ancestry<grandchild_target_type>::value);
        REQUIRE(concepts::machine_ancestry<grandchild_target_type>);
      }
    }
  }
}