...
template <typename> struct is_transition_coordinator : std::false_type {};

template <typename T>
requires concepts::state_provider<T> && concepts::event_engine<T> &&
    concepts::transition_table<typename T::transition_table_type> &&
    requires(T &t) {
  { t.transition_table() }
  noexcept->std::same_as<
      std::add_lvalue_reference_t<typename T::transition_table_type>>;
}
struct is_transition_coordinator : std::true_type {};

namespace concepts {
template <typename T>
concept transition_coordinator = is_transition_coordinator<T>::value;
} // namespace concepts
...
//     unique_t<map_t<
//         filter_t<TransitionTable, negate<is_self_transition>::template
//         apply>, next_state_t>>,
//     states_list>;
...
//   State> constexpr void schedule_entry() noexcept {
